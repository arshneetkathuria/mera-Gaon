...
List<File> _images=[];
          ...
          GestureDetector(
              onTap: () async {
                ...
                Navigator.push(context,
                    MaterialPageRoute(builder: (context) =>Personalities()));
              },
              child: Padding(
  ...
  Future _uploadPhoto(String key) async
  {
    ...
    for(var img in _images)
    {
      ...
      await ref.getDownloadURL().then((value) => SignUp.fb.reference().child(key).child("FineArts").child("images").child("FineArtsImage$i").set(value));
      i++;
      print("uploaded");
    }
}
...
{
  SignUp.fb.reference().child(key).child("FineArts").child("titleForFineArts").set(title.text);
 return _uploadPhoto(key);
}
